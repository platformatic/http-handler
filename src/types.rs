...
pub type Request<T = Bytes> = http::Request<RequestBody<T>>;

/// Type alias for HTTP Response with streaming body
pub type Response<T = Bytes> = http::Response<ResponseBody<T>>;
...
mod tests {
    ...
    fn test_response_type_alias() {
        let request_body = RequestBody::<Bytes>::new();
        let (response_body, _tx) = request_body.create_response();
