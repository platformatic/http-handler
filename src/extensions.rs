...
impl<T> ResponseExt for http::Response<T> {
    ...
    fn set_exception(&mut self, exception: impl Into<String>) {
        self.extensions_mut().insert(ResponseException::new(exception));
    }
}
...
mod tests {
    ...
    fn test_response_ext() {
        ...
        assert_eq!(response.log().unwrap().as_bytes(), b"Initial log - more data");

        // Set exception
        response.set_exception("Something went wrong");
        assert_eq!(response.exception().unwrap().message(), "Something went wrong");
