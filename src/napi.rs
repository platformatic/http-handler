...
use crate::{
    RequestBody, RequestBuilderExt, RequestExt,
    ResponseBuilderExt, ResponseExt, SocketInfo as InnerSocketInfo, WebSocketMode,
};
...
impl Request {
    ...
    pub async fn write(&self, chunk: Either<Buffer, String>) -> Result<()> {
        ...
        if self.0.extensions().get::<crate::extensions::BodyBuffer>().is_some() {
...
impl Response {
    ...
    pub fn new(options: Option<ResponseOptions>) -> Result<Self> {
        ...
        // Create response body with channels
        // For NAPI Response construction, we create both request body (unused) and response body
        let request_body = RequestBody::new();
        let (response_body, response_tx) = request_body.create_response();

        // If body data is provided, send it through the response channel AND store for getter
        let buffered_body = if let Some(body_buf) = options.body {
            let bytes = Bytes::copy_from_slice(body_buf.as_ref());
            // Use try_send for synchronous constructor
            response_tx
                .try_send(Ok(crate::body::StreamChunk::Data(bytes.clone())))
                .map_err(|_| Error::from_reason("Failed to send response body"))?;
            // Send End marker
            response_tx
                .try_send(Ok(crate::body::StreamChunk::End))
                .map_err(|_| Error::from_reason("Failed to send End marker"))?;
            Some(bytes)
        } else {
            None
        };
    ...
    pub fn make_streamable(self, env: Env) -> Result<Object<'static>> {
        ...
        napi::check_status!(
            unsafe {
                sys::napi_set_property(raw_env, response_js_value, iterator_symbol, generator_function)
            },
        ...
        match self.0.body_mut().frame().await {
            Some(Ok(frame)) => {
                ...
                if let Ok(data) = frame.into_data() {
                    ...
                    // Frame was trailers or empty, try next frame
                    Ok(None)
                }
            }
            Some(Err(e)) => {
...
impl AsyncGenerator for Response {
    ...
    fn next(&mut self, _value: Option<Self::Next>) -> impl Future<Output = Result<Option<Self::Yield>>> + Send + 'static {
        ...
        let body: &'static mut crate::ResponseBody<Bytes> = unsafe {
            std::mem::transmute(self.0.body_mut())
        };
