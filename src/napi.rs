...
use std::{
  collections::HashMap, net::SocketAddr, ops::{Deref, DerefMut}
};

use crate::{Request, RequestBuilderExt, Response, ResponseBuilderExt, SocketInfo};
...
impl TryFrom<SocketInfo> for NapiSocketInfo {
  ...
  fn try_from(socket: SocketInfo) -> Result<Self> {
    let local = socket.local.ok_or(
      Error::new(Status::InvalidArg, "Local socket address is required")
    )?;
    let remote = socket.remote.ok_or(
      Error::new(Status::InvalidArg, "Remote socket address is required")
    )?;

    fn socket_info_tuple(socket: &SocketAddr) -> (String, u16, String) {
      (socket.ip().to_string(), socket.port(), if socket.is_ipv4() {
        "IPv4"
      } else {
        "IPv6"
      }.to_string())
...
impl TryFrom<NapiSocketInfo> for SocketInfo {
  ...
  fn try_from(socket: NapiSocketInfo) -> std::result::Result<Self, Self::Error> {
    ...
    let local = sock_addr(&socket.local_family, &socket.local_address, socket.local_port)?;
    let remote = sock_addr(&socket.remote_family, &socket.remote_address, socket.remote_port)?;
    Ok(Self {
      local: Some(local),
      remote: Some(remote),
    })
...
impl FromNapiValue for NapiHeaders {
  unsafe fn from_napi_value(env: sys::napi_env, value: sys::napi_value) -> Result<Self> {
    ...
    Err(Error::new(Status::InvalidArg, "Expected Headers or NapiHeaderMap"))
  }
}
...
impl NapiHeaders {
  ...
  /// Append a value to a header entry.
  ///
  /// # Examples
  ///
  /// ```js
  ...
  /// ```
  #[napi]
  pub fn append(&mut self, key: String, value: String) -> Result<bool> {
    let key = HeaderName::try_from(key)
      .map_err(|e| Error::new(Status::InvalidArg, format!("Invalid header name: {}", e)))?;
...
impl NapiResponse {
  ...
  pub fn new(options: NapiResponseOptions) -> Result<Self> {
    ...
    let response = builder
      .body(body)
      .map_err(|e| Error::new(Status::InvalidArg, format!("Failed to build response: {}", e)))?;
